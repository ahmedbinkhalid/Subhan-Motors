...
export const GetApprovedBlogs: React.FC<GetApprovedBlogsProps> = ({
  title,
}) => {
    ...
    <React.Fragment>
      <section className="lg:max-w-3xl xl:max-w-5xl mx-auto">
        <div className="flex justify-center items-center gap-4 md:text-2xl text-xl text-charcoal-gray font-semibold font-sans xxl:mt-4 mb-8 md:mt-2">
          <h1 className="">{!title ? "All your Blog's" : title}</h1>
        ...
        <section className="bg-slate-50 md:p-8 p-4 border-y-2  flex flex-col">
          ...
          <div className="flex flex-wrap justify-center md:justify-end items-center mt-8 gap-2">
            <button
              className={`bg-regal-red hover:bg-red-700 text-white py-2 px-4 rounded ${
            ...
            <div className="flex flex-wrap justify-center gap-1">
              {Array.from(
                { length: Math.ceil(blogCardsData.length / itemsPerPage) },
                (_, index) => (
                  <button
                    key={index}
            ...
            <button
              className={`bg-regal-red hover:bg-red-700 text-white py-2 px-4 rounded ${
                currentPage === Math.ceil(blogCardsData.length / itemsPerPage)
              ...
              onClick={() =>
                setCurrentPage((prev) =>
                  Math.min(
                    prev + 1,
                    Math.ceil(blogCardsData.length / itemsPerPage)
                  )
                )
              }
              disabled={

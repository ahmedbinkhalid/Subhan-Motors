...
import { PhotoLimitationsProps, UploadPhotosProps } from "./types";

export const UploadPhotos: React.FC<UploadPhotosProps> = ({ bgColor, hoverBgColor }) => {
  const { images, setImages } = useImageContext(); 
  const [loading, setLoading] = useState<Record<number, boolean>>({});

  const PhotoLimitations: React.FC<PhotoLimitationsProps> = ({ limitation }) => (
    <p className="flex gap-2">
      <IoMdCheckmarkCircleOutline size={20} className={`object-cover text-${bgColor} mt-[1.5px] font-bold`} />
  ...
  return (
    <React.Fragment>
      ...
      <div className={`border-2 border-dashed border-${bgColor}  p-4 `}>
        <div className="flex md:gap-4 gap-2 justify-center items-center w-full">
          <div className="p-4 rounded-full bg-slate-100 border-[0.5px]">
            <IoMdPhotos size={52} className={`object-cover text-${bgColor}`} />
          </div>

          <div className="flex flex-col gap-1 self-end">
            <label htmlFor="file-upload" className="cursor-pointer">
              <div className={`bg-${bgColor} hover:bg-${hoverBgColor} py-2 md:px-6 px-3 rounded-lg text-center md:text-base text-sm text-white font-semibold h-fit self-center flex gap-2`}>
        ...
        <div className="grid 2xl:grid-cols-6 lg:grid-cols-5 md:grid-cols-3 grid-cols-1 gap-4 my-6 max-md:flex max-md:flex-col max-md:items-center max-md:justify-center">
          {images.map((image, index) => (
            <div key={index} className="relative w-40 h-40 bg-gray-100 rounded-md overflow-hidden flex justify-center items-center">
              ...
              >
                <LuImageMinus size={20} className={`${bgColor}`} />
              </button>
            </div>
          ))}
        </div>

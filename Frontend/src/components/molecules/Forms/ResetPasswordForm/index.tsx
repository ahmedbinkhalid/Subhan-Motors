...
export const OtpForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!otpToken) {
      setMessage(
        "OTP token is missing. Please initiate the password reset process again."
      );
      return;
    }
  ...
  return (
    <div className="flex flex-col md:gap-4 gap-1">
      <img src={OTP} alt="Reset Password" className="object-contain h-48" />
      <h1 className="md:text-xl text-lg  self-center font-bold leading-none">
        Resetting Your Password!
      </h1>
      <p className="self-center text-lg font-medium leading-none text-center">
        {" "}
        We Help you Resetting your Password{" "}
      </p>
      <form
        onSubmit={handleSubmit}
        className=" flex flex-col md:gap-4 gap-1 pb-2"
      >
